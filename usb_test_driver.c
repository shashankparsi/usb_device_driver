...
static ssize_t usb_test_read(struct file *filp,char __user *user_buffer,size_t count,loff_t *offset)
{
...
static ssize_t usb_test_write(struct file *filp,const char __user *user_buffer,size_t count,loff_t *offset)
{
pr_info("%s: in write function\n",__func__);
int retval;
//int write_cnt = MIN(count,(MAX_PKT_SIZE));
int write_cnt;
...
{
write_cnt=MAX_PKT_SIZE;
}
if(copy_from_user(bulk_buf,user_buffer,write_cnt))
{
return -EFAULT;
}
retval=usb_bulk_msg(device,usb_sndbulkpipe(device,BULK_EP_OUT),bulk_buf,count,&write_cnt,5000);
if(retval)
{
